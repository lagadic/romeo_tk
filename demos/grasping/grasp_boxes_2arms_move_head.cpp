...
int main(int argc, const char* argv[])
{
    ...
    std::string opt_box_name1 = "tabascobox_green";
    ...
    {
        state_teabox_tracker[0] = TrackColorObject;
        state_teabox_tracker[1] = TrackColorObject;
    }
    ...
    hand_tracker_r.setLeftHandTarget(true);
    ...
    std::vector<vpColorDetection*> obj_color;

    vpColorDetection obj_color_l;
    obj_color_l.setName(opt_box_name);
    std::string filename_color_l = box_folder + "color/" + opt_box_name + "HSV.txt";
    if (!obj_color_l.loadHSV(filename_color_l))
    {
        std::cout << "Cannot load file " << filename_color_l << std::endl;
        return 0;
    }

    vpColorDetection obj_color_r;
    obj_color_r.setName(opt_box_name1);
    std::string filename_color_r = box_folder + "color/" + opt_box_name1 + "HSV.txt";
    if (!obj_color_r.loadHSV(filename_color_r))
    {
        std::cout << "Cannot load file " << filename_color_r << std::endl;
        return 0;
    }

    obj_color.push_back(&obj_color_l);
    obj_color.push_back(&obj_color_r);

    std::vector<bool> obj_color_found(2);
    obj_color_found[0] = false;
    obj_color_found[1] = false;


    std::cout << "Color1 " << obj_color[0]->getValueHSV() << std::endl;

    std::cout << "Color2 " << obj_color[1]->getValueHSV() << std::endl;
    /************************************************************************************************/
    ...
    std::string learned_oMh_filename = "grasping_pose2arms_blob.xml";
    ...
    while(1) {
        ...
        /*    cv::imshow("left", cvI[0]);
    cv::imshow("right", cvI[1])*/;
        ...
        for(unsigned int i = 0; i < 2 ; i++)
        {
            ...
            if(state_teabox_tracker[i] == TrackColorObject) {

                vpDisplay::displayText(I[i], vpImagePoint(10,10), "Color detection. Click left to continue", vpColor::red);

                //     if(i == 0)
                //        obj_color_found[i] =  obj_color[0]->detect(cvI_l);
                //     else
                //       obj_color_found[i] =  obj_color[1]->detect(cvI_r);


                if(i == 0)
                    obj_color_found[i] =  obj_color_l.detect(cvI_l);
                else
                    obj_color_found[i] =  obj_color_r.detect(cvI_r);

                if (obj_color_found[i]) {
                    ...
                    vpRect bbox =  obj_color[i]->getBBox(j);
                    vpDisplay::displayRectangle(I[i], bbox, vpColor::yellow, false, 1);
                    vpDisplay::displayText(I[i], (int)bbox.getTop()-10, (int)bbox.getLeft(), obj_color[i]->getMessage(j), vpColor::yellow);

                    vpImagePoint head_cog_cur = obj_color[i]->getCog(j);
                ...
                if (i && obj_color_found[0] && obj_color_found[1])
                {
           ...
           // status_hand_tracker[i] = hand_tracker[i]->track(cvI[i],I[i]);
            ...
            if (status_hand_tracker[i]) { // display the tracking results
                ...
                // vpDisplay::displayPolygon(I[i], hand_tracker[i]->getCorners(), vpColor::green, 2);
            }
            ...
            if (state_teabox_tracker[i] == WaitGrasping && status_hand_tracker[i] && status_teabox_tracker[i]) {
            ...
            if (state_teabox_tracker[i] == Grasping) {
                if (status_hand_tracker[i] && status_teabox_tracker[i]) {
                    ...
                    if (! grasp_servo_converged[i]) {
                        ...
                        cdMc[i] = (cMo_hand[i].inverse() * cMo_teabox[i] * oMh_Tea_Box_grasp[i]).inverse();
