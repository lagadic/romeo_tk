...
int main(int argc, const char* argv[])
{
  ...
  std::cout << "Opt_model: " << opt_model << std::endl
            << "config_detection_file_folder: " << config_detection_file_folder << std::endl;
  ...
  while(1) {
    ...
    if (status_template_tracker ) { // display the tracking results
      ...
      printPose("cMo box: ", cMo_t);
      // The qrcode frame is only displayed when PBVS is active or learning

      vpDisplay::displayFrame(I, cMo_t, cam, 0.04, vpColor::none, 1);
